...
//FIXME: Big endian systems
uint64_t shift(uint32_t structure, uint16_t colors, uint8_t children, uint8_t size)
{
    uint64_t r = size; //size goes in the
    r = (r<<8) + children;
    r = (r<<16) + colors;
    r = (r<<32) + structure;
...
{
    CHECK(Treelet::singleton(0)  == shift(0b00000000000000000000000000000000, 0b0000000000000001, 0, 1));
    CHECK(Treelet::singleton(1)  == shift(0b00000000000000000000000000000000, 0b0000000000000010, 0, 1));
    CHECK(Treelet::singleton(2)  == shift(0b00000000000000000000000000000000, 0b0000000000000100, 0, 1));
    CHECK(Treelet::singleton(15) == shift(0b00000000000000000000000000000000, 0b1000000000000000, 0, 1));
}
...
TEST_CASE("treelet merges")
{
    ...
    CHECK(t0_1 == shift(0b10000000000000000000000000000000, 0b0000000000000011, 1, 2));
    ...
    CHECK(t0_12 == shift(0b10100000000000000000000000000000, 0b0000000000000111, 2, 3));
    ...
    CHECK(t0_123 == shift(0b10101000000000000000000000000000, 0b0000000000001111, 3, 4));

    //A path 3--4
    Treelet::treelet_t t3_4 = Treelet::merge(t3, t4);
    CHECK(t3_4 == shift(0b10000000000000000000000000000000, 0b0000000000011000, 1, 2));

    //A star with an extra leaf, 0--1, 0--2, 0--3, 3--4
    Treelet::treelet_t t0_123_4 = Treelet::merge(t0_12, t3_4);
    CHECK(t0_123_4 == shift(0b11001010000000000000000000000000, 0b0000000000011111, 1, 5));

    //A path 2--3, 3--4
    Treelet::treelet_t t2_3_4 = Treelet::merge(t2, t3_4);
    CHECK(t2_3_4 == shift(0b11000000000000000000000000000000, 0b0000000000011100, 1, 3));

    //A spider of height 2 with two legs 2--3, 3--4, 2--0, 0--1
    Treelet::treelet_t t2_3_4__0_1 = Treelet::merge(t2_3_4, t0_1);
    CHECK(t2_3_4__0_1 == shift(0b11001100000000000000000000000000, 0b0000000000011111, 2, 5));
    ...
    CHECK(t5_6 == shift(0b10000000000000000000000000000000, 0b0000000001100000, 1, 2));
    ...
    CHECK(t2_3_4__0_1__5_6== shift(0b11001100110000000000000000000000, 0b0000000001111111, 3, 7));

    //A path 5--2, 2--3, 3--4
    Treelet::treelet_t t5_2_3_4 = Treelet::merge(t5, t2_3_4);
    CHECK(t5_2_3_4 == shift(0b11100000000000000000000000000000, 0b0000000000111100, 1, 4));
    ...
    CHECK(t0_16 == shift(0b10100000000000000000000000000000, 0b0000000001000011, 2, 3));
