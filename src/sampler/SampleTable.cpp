...
SampleTable::SampleTable(Occurrence *occurrences, uint64_t noccurrences, TreeletSelector *ts) {
	...
	if (ts != nullptr && ts->get_treelet_size() == k && ts->get_size() == 2
			&& ts->get_treelets()[0].is_star() && ts->get_treelets()[1].is_star()) {
	...
	google::dense_hash_map<Occurrence*, uint64_t, Occurrence::OccurrenceFootprintHash,
			Occurrence::OccurrenceFootprintEquality> ht(noccurrences);
	...
	for (const auto& kv : ht) {
		...
		normalized_samples += static_cast<double>(e.sample_count)
				/ static_cast<double>(e.num_spanning_trees);
		entries.push_back(e);
	}
 ...
 */
void SampleTable::estimateOccurrences(double num_graph_treelets, unsigned int k,
		bool store_only_0) {
	for (auto &e : entries)
		e.estimate_graph_occurrences = (static_cast<double>(e.sample_count)
...
SampleTable SampleTable::merge(SampleTable& t1, SampleTable& t2, double tcount1, double tcount2) {
	...
	for (auto& kv : merged) {
		...
		e.estimate_graph_occurrences = static_cast<double>(e.sample_count)
				/ (static_cast<double>(s) * weights[kv.first]);
