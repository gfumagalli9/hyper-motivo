			...
			: graph(graph), size(size), ttc(ttc), number_of_threads(number_of_threads), store_only_on_0(store_only_on_0)
{
	...
	for (const auto &[structure, count] : numTreelets2) // cumulate each treelet's count to its representant's count
	{
	...
	for (const Occurrence &j : completedGraphlets)
	{
		...
		for(const auto &[spanning_repr, noccurences]: occurrences_to_spanning_representants[j])
		{
			...
			if (num_it != numTreelets.cend())
				treeletInefficiency[spanning_repr] += ( static_cast<double>(noccurences) * static_cast<double>(occ_info.num_occurrences)) / (static_cast<double>(occ_info.weight) * static_cast<double>(num_it->second));
		}
	}
...
SampleTable* AdaptiveSampler::sample(const uint64_t n_samples, Random* rng, const double time_budget)
{
	...
	while ((samples_rem > 0 || n_samples == 0) && totTime < time_budget) // take suffSamples more samples, in parallel
	{
		...
		//FIXME: representant_to_containing_occurrences and occurrences_to_spanning_representants need to be filled somewhere...
		...
		{
			double noccs = occurrences_to_spanning_representants[occ][current_treelet_structure];
	...
	std::cout << "time spent in sampling treelets: " << sampleTime << std::endl;
	...
	{
		SampleTable::Entry e;
		e.occurrence = it.first;
		e.fingerprint = (it.first.text_footprint());
		e.num_spanning_trees = 0;
		e.sample_count = it.second.num_occurrences;
		e.estimate_graph_occurrences = static_cast<double>(it.second.num_occurrences) * (store_only_on_0 ? size : 1) / (it.second.weight * p);
		table->addEntry(e);
	}
	std::cout << "total management time: " << totManagementTime << std::endl;
	table->estimateFrequencies();
	std::cout << "norm-2 of the sample distribution: " << table->norm2() << std::endl;
	return table;
}
