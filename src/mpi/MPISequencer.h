...
class MPISequencer : public BaseSequencer<UndirectedGraph::vertex_t>
{
private:
...
public:
    ...
    BaseSequencer<UndirectedGraph::vertex_t>::sequence_batch_t next_batch()
    {
        BaseSequencer<UndirectedGraph::vertex_t>::sequence_batch_t batch;
        ompi_status_public_t status;

        context->lock();
        MPI_Send(nullptr, 0, MPI_BYTE, sequencer_server, protocol::MSG_NEXT_BATCH_REQUEST, commuicator);
        MPI_Recv(&batch, sizeof(BaseSequencer<UndirectedGraph::vertex_t>::sequence_batch_t), MPI_BYTE, sequencer_server, protocol::MSG_NEXT_BATCH_RESPONSE, commuicator, &status);
        context->unlock();
