/*
 * SimpleTreeletTableBuilder.cpp
 *
 *  Created on: 15 mag 2018
 *      Author: brix
 */

#include "SimpleTreeletTableBuilder.h"
#include <sstream>

SimpleTreeletTableBuilder::SimpleTreeletTableBuilder(const UndirectedGraph* graph,
		const GraphColoring* coloring, const unsigned int size, const TreeletTableCollection* lower,
		std::ostream* output, const bool store_0_only, TreeletSelector* selector) :
		graph(graph), coloring(coloring), size(size), lower(lower), output(output), store_0_only(
...
void SimpleTreeletTableBuilder::do_build_1_st()
{
	...
	for (UndirectedGraph::vertex_t u = 0; u < graph->number_of_vertices(); u++) {
		...
		tcp.treelet = Treelet::singleton(u);
...
std::pair<char*, std::size_t> SimpleTreeletTableBuilder::to_normalized_sorted_byte_array(
		const UndirectedGraph::vertex_t u, const table_t &table) {
	std::pair<char*, std::size_t> result;
	result.second = sizeof(UndirectedGraph::vertex_t) + sizeof(uint64_t)
			+ table.size() * sizeof(TreeletTable::treelet_count_pair);
	...
	TreeletTable::treelet_count_pair *counts =
			new (result.first + sizeof(UndirectedGraph::vertex_t) + sizeof(uint64_t)) TreeletTable::treelet_count_pair[size];
	...
	std::sort(counts, counts + table.size(),
			[](const TreeletTable::treelet_count_pair& tc1, const TreeletTable::treelet_count_pair& tc2) {return tc1.treelet < tc2.treelet;});
	return result;
}

void SimpleTreeletTableBuilder::combine(const UndirectedGraph::vertex_t u,
		const UndirectedGraph::vertex_t v, table_t& counts) {
